...
export function setupSceneGameApi() {
  ...
  window.GameAPI = {
    onBeat: () => {
      ...
      const baseSpeed = (ENEMY_SPEED_BASE + Math.floor(combat.beats / 16) * 30) * currentDifficulty.speedMult;
    ...
    onSnare: () => {
      ...
      const baseSpeed = (ENEMY_SPEED_BASE * 1.5 + Math.floor(combat.beats / 16) * 30) * currentDifficulty.speedMult;
      const speed = baseSpeed * this.adaptiveState.currentSpeedMultiplier;
      const lane = Phaser.Math.Between(0, LANES - 1);
